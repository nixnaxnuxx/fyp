(() => {
  ...
  function meetingsView(){
    if(isAdmin()){
      const rows=state.meetingSlots.map(slot=>{const b=bookingForSlot(slot.id),stu=b?state.students.find(s=>s.id===b.student_id):null,[lab,cl]=meetingStatus(slot),rec=b?meetingRecordForBooking(b.id):null,openF=b?followupsForBooking(b.id).filter(f=>f.status!=='done').length:0;return `<tr><td>${fmt(slot.start_at)}</td><td>${fmt(slot.end_at)}</td><td>${esc(slot.location||'')}</td><td><span class="pill ${cl}">${lab}</span></td><td>${b?`${esc(stu?.full_name||'')}<div class="tiny muted">${esc(projectForStudent(stu?.id)?.title||'')}</div>`:'—'}</td><td>${b?`<button class="btn ${rec?'secondary':''} small recordMeeting" data-booking="${b.id}">${rec?'View / Update Notes':'Record Meeting'}</button>${openF?`<div class="tiny muted" style="margin-top:6px">${openF} follow-up${openF===1?'':'s'} open</div>`:''}`:'—'}</td></tr>`}).join('');
      return top('FYP Supervision Meetings','Set availability, record what was discussed, and assign actions for the next supervision meeting.',`<div class="top-actions"><button class="btn secondary" id="emailStudents">Email All Students</button><button class="btn" id="newMeetingSlot">+ Add Availability</button></div>`)+
      `<div class="card"><h3 style="margin-top:0">Student & project list</h3>${studentProjectList()}</div><div class="section-title"><h2>Availability, bookings & supervision notes</h2></div><div class="table-wrap"><table class="table"><thead><tr><th>Start</th><th>End</th><th>Location / Mode</th><th>Status</th><th>Booked by</th><th>Supervision record</th></tr></thead><tbody>${rows||'<tr><td colspan="6" class="muted">No availability slots yet. Add the times you are free this week.</td></tr>'}</tbody></table></div>`;
    }
