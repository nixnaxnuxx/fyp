(() => {
  ...
  const state = { user:null, profile:null, view:'dashboard', cohorts:[], students:[], projects:[], tasks:[], assignments:[], submissions:[], feedback:[], files:[], studentRecord:null };
  ...
  async function loadData(){
    ...
    const [coh,stu,pro,tas,ass,sub,fee] = await Promise.all(fetches);
    state.cohorts=coh.data||[]; state.students=stu.data||[]; state.projects=pro.data||[]; state.tasks=tas.data||[]; state.assignments=ass.data||[]; state.submissions=sub.data||[]; state.feedback=fee.data||[];
  ...
  function nav(){
    const items = isSupervisor() ? [['dashboard','Dashboard'],['students','Students'],['tasks','Tasks'],['submissions','Submissions'],['progress','Progress'],['cohorts','Cohorts'],['export','Export']] : [['dashboard','My FYP'],['tasks','My Tasks'],['submissions','My Submissions'],['progress','Progress'],['export','Export Record']];
  ...
  function bindViewActions(){
    const ns=q('#newStudent');if(ns)ns.onclick=openStudentModal; const nt=q('#newTask');if(nt)nt.onclick=openTaskModal; const nc=q('#newCohort');if(nc)nc.onclick=openCohortModal;
